...
class CronController extends Controller
{
    ...
    */
    ...
    public function sendNotification(Record $record, $count)
    {
        ...
        $html = '';
        if ($count == 1){
            $html = 'Здравейте,<br/>Напомняме Ви, че имате нова заявка, за която клиент очаква вашата реакция. Моля влезте в системата, за да я обработите. За по-лесен достъп може да проследите посочения линк:<br/><a href="' . route('records.edit', $record) . '">Кликнете тук за да видите детайлите</a>.<br/>Поздрави,<br/>Екипът на Метрика';
            $subject = 'Напомняне за НЕПРИЕТА – нова заявка';
        }elseif ($count == 2){
            $html = 'Здравейте,<br/>Имате заявка в системата, на изчакване 24часа. Моля влезте в системата, за да я обработите. За по-лесен достъп може да проследите посочения линк:<br/><a href="' . route('records.edit', $record) . '">Кликнете тук за да видите детайлите</a>.<br/>Поздрави,<br/>Екипът на Метрика';
